...
vignette: >
  %\VignetteIndexEntry{The `FF16` model}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

- **authors**: DS Falster, RG FitzJohn, Å Brännström, U Dieckmann, M Westoby
- **date**: 2016
...
get_file <- function(file) {
  here::here(file.path("vignettes/models/FF16", file))
}
```
...
knitr::include_graphics(get_file("FF16-schematic-phys.png"))
```
...
here::here("vignettes/demography-vars.csv") %>%
...
x <- get_file("FF16-params-core.csv") %>%
  read.csv() %>% dplyr::as_tibble()
...
x <- get_file("FF16-params-hyper.csv") %>%
  read.csv() %>% dplyr::as_tibble() 
